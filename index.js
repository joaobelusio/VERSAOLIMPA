...
/**
 * *** MUDANÇA IMPORTANTE ***:
 * Inserimos a instrução para que o GPT, se perceber que faltam dados cruciais,
 * PERGUNTE ao usuário (e retorne operation=NONE) ao invés de gerar INSERT incompleto.
 */
const systemPrompt = `
Você é um assistente que gerencia estoque e vendas de produtos à base de CBD em um banco de dados PostgreSQL.

Regras IMPORTANTES:
1) Se tiver dados suficientes (brand, product_name, quantity, operation_type) gere o JSON. Se "patient_name" não vier, pode ficar nulo. Não bloqueie.
2) Se faltar dado essencial (por ex. não sabe se é ENTRADA ou SAÍDA), pergunte. Use "operation":"NONE".
3) Retorne SEMPRE JSON válido com "operation", "table", "fields"/"where".
4) Se for SELECT, retorne "operation":"SELECT", "table":"inventory" ou "transactions", etc.
5) Se for inserir ou atualizar, "operation_type" = "ENTRADA" ou "SAÍDA" (para transações).
6) Se o usuário diz "vender" ou "quero X", interprete como SAÍDA. Se diz "adicionar", interprete como ENTRADA. Se tiver dúvida, pergunte.
7) "brand" e "product_name" devem ser separados. Se o usuário disser "1drop 6000", tente converter brand= "1DROP", product_name="1Drop 6000mg CBD Isolado 30ml" (se for esse). Caso não tenha certeza, pergunte.
8) Retorne a resposta em português e ao final um bloco JSON. Se não precisar de operação, "operation":"NONE".
...
function extractJsonBlock(fullAnswer) {
  // 1) Tenta com crases (backticks) usando uma expressão mais ampla:
  const tripleRegex = /```(?:json)?\s*([\s\S]*?)```/;
...
function fixJsonCommonIssues(jsonStr) {
  // substitui "product" por "product_name", se existir
  // *** MUDANÇA *** Para corrigir a questão de "product" vs "product_name"
  let fixed = jsonStr.replace(/"product"\s*:/gi, '"product_name":');

  // substitui "table":"stock" por "table":"inventory"
  fixed = fixed.replace(/"table":"stock"/gi, '"table":"inventory"');

  // substitui cost_in_usd -> cost_in_dollar
  fixed = fixed.replace(/cost_in_usd/gi, 'cost_in_dollar');
  
  // caso existam aspas duplas mal formatadas, etc, podemos tentar correções adicionais
...
async function executeDbOperation(jsonStr) {
  ...
  // antes de disparar para DB, se for transação, tentamos heurísticas
  if (operation === 'INSERT' && table === 'transactions') {
    // *** MUDANÇA ***: Tentar inferir operation_type se não vier
    if (!fields.operation_type) {
      // Heurística: se o texto do usuário tinha "adicionar", ENTÃO "ENTRADA",
      // senão se tinha "vender", "comprar" ou "quero X frascos", ENTÃO "SAÍDA"
      // (esta heurística poderia ser refinada)
      const lastUserMsg = (await getConversationHistory(table))?.slice(-1)?.[0]?.content || '';
      // Mas aqui, simplifico e assumo "SAÍDA"
      fields.operation_type = "SAÍDA";
    }
  }
...
function stringSimilarity(a, b) {
  ...
  // Pode ser uma lógica mais elaborada, mas aqui apenas um count simples
  let matches = 0;
  for (let ch of bb) {
...
async function insertTransaction(fields) {
  ...
  // *** MUDANÇA ***: brand e product_name obrigatórios,
  // mas se não vier brand, tentamos separar "product_name"
  if (!brand && product_name) {
    // Exemplo de heurística: se "product_name" começar com "1drop",
    // definimos brand = "1DROP". Caso contrário, pedimos fuzzy. (Simplificado!)
    const matchBrand = (product_name || '').match(/^(\S+)/);
    if (matchBrand) {
      brand = matchBrand[1].toUpperCase();
    }
  }

  if (!brand || !product_name || !quantity) {
    return 'Faltam campos obrigatórios (brand, product_name ou quantity).';
  }

  // operation_type - se não vier, definimos "SAÍDA"
  if (!operation_type) {
    operation_type = 'SAÍDA';
  }

  // Faz fuzzy matching do product_name
  const bestName = await findClosestProductName(brand, product_name);
  if (bestName) {
    product_name = bestName;
  }

  // Se tem patient_name, buscar. Se não, patient_id = null
  let patientId = null;
  if (patient_name) {
    const pat = await pool.query(`
      SELECT id FROM patients WHERE full_name ILIKE $1
    `, [patient_name]);
    if (!pat.rows.length) {
      // Cria ou retorna erro? Aqui simplesmente definimos nulo, ou poderíamos criar
      // return `Paciente '${patient_name}' não encontrado. Cadastre antes ou corrija o nome.`;
      // *** MUDANÇA ***: setar nulo e seguir
      patientId = null;
    } else {
  ...
  if (inv.rows.length) {
    ...
    if (operation_type === 'ENTRADA') {
      ...
      newQty = current - quantity;
      if (newQty < 0) newQty = 0; // ou poderia bloquear
    }
    await pool.query(`
    ...
    newQty = (operation_type === 'ENTRADA') ? quantity : 0;
  }
